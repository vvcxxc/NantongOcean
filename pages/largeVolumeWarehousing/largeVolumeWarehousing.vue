...
<script>
	...
	export default {
		...
		methods: {
			...
			getBarCodeInfos() {
				// console.clear();
				// this.qqq.isGetBarCodeInfos = parseInt(Math.random() * (100 - 1 + 1) + 1);
				this.tableItem = {
					value: '',
					Length: '',
					Oid: '',
				...
				PDA_BindBarCodeData({
					...
					if (res.responseCode == 0) {
						// 获取已组盘数据明细
						console.log('获取已组盘数据明细', res.BindBarCodeInfos);
						// BindBarCodeInfos[0] 拿最新的
		...
		onShow: function() {
			// this.qqq.isOnShow = parseInt(Math.random() * (100 - 1 + 1) + 1);
			// console.log('onShow', this.qqq.onShow);
			this.getBarCodeInfos();
		},
		beforeMount: function() {
			// this.qqq.isBeforeMount = parseInt(Math.random() * (100 - 1 + 1) + 1);
			// console.log('载入：beforeMount', this.qqq.isBeforeMount);
			this.getBarCodeInfos();
		},
		mounted() {
			// this.qqq.isMounted = parseInt(Math.random() * (100 - 1 + 1) + 1);
			// console.log('载入：mounted', this.qqq.isMounted);
			this.getBarCodeInfos();
		}
	};
</script>
